...
describe('User Entity', () => {
  it('hashes password and validates fields', () => {
    const user = new User('u1', 'johnsmith', 'password123', 'USER', null, null, new Date(), new Date(), null);
  ...
  it('throws on invalid username length', () => {
    expect(() => new User('u1', 'ab', 'password123', 'USER', null, null, new Date(), new Date(), null)).toThrow();
  });

  it('throws on invalid password length when not hashed', () => {
    expect(() => new User('u1', 'validname', 'short', 'USER', null, null, new Date(), new Date(), null)).toThrow();
  });

  it('accepts pre-hashed passwords when isHashed flag is true', () => {
    const preHashed = 'a'.repeat(64);
    const user = new User('u1', 'validname', preHashed, 'USER', null, null, new Date(), new Date(), null, { isHashed: true });
    expect(user.password).toBe(preHashed);
  });

  it('throws on invalid role', () => {
    expect(() => new User('u1', 'validname', 'password123', 'INVALID' as any, null, null, new Date(), new Date(), null)).toThrow(
    );
  });

  it('skipValidation allows invalid lengths and roles (still hashes when not isHashed)', () => {
    const user = new User('u1', 'ab', 'short', 'INVALID' as any, null, null, new Date(), new Date(), null, { skipValidation: true });
    expect(user.password).toHaveLength(64);
  });

  it('hashPassword is deterministic and different from plaintext', () => {
    const u = new User('u1', 'johnsmith', 'password123', 'USER', null, null, new Date(), new Date(), null);
