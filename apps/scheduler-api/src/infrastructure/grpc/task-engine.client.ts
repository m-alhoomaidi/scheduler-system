...
import {
  ...
  TRegisterTaskInput,
  TRegisterTaskOutput,
} from '@/domain/ports/task-engine.port';
...
export class TaskEngineGrpcClient implements TaskEnginePort, OnModuleInit {
  ...
  constructor(
    @Inject('TASK_ENGINE_GRPC') private readonly client: ClientGrpc,
  ) {}
  ...
  async registerTask(input: TRegisterTaskInput): Promise<TRegisterTaskOutput> {
    ...
    const res: RegisterTaskResponse = await lastValueFrom(
      this.svc.registerTask(req),
    );
  ...
  async deleteTask(taskId: string): Promise<{ ok: boolean }> {
    ...
    const res: DeleteTaskResponse = await lastValueFrom(
      this.svc.deleteTask(req),
    );
