...
import {
  DispatchScheduledTaskParams,
  EnqueueScheduledTaskParams,
...
export class ScheduledTaskQueueRepository extends ScheduledTaskQueueRepositoryPort {
  constructor(
    @InjectModel('ScheduledTaskQueue')
    private readonly scheduledTaskQueueModel: Model<ScheduledTaskQueueDocument>,
  ) {
    super();
  }
  ...
  async enqueue(
    entry: EnqueueScheduledTaskParams,
  ...
  async markDispatched(entry: DispatchScheduledTaskParams): Promise<void> {
    await this.scheduledTaskQueueModel.updateOne(
      { id: entry.id },
      { $set: { isDispatched: true, dispatchedAt: entry.dispatchedAt } },
    );
  }

  async findPending(limit?: number): Promise<ScheduledTaskQueue[]> {
    const docs = await this.scheduledTaskQueueModel
